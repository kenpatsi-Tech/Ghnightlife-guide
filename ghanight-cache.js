...
// ============================================================
// GhaNight Rate Guard
// Prevents hammering Supabase during viral moments and
// prevents ticket overselling race conditions.
// ============================================================

const GNRateGuard = (function() {

  // ── Client-side rate limiting ─────────────────────────────
  const clientLimits = new Map();

  function checkLimit(action, maxPerWindow, windowMs) {
    const key = 'rg_' + action;
    const now = Date.now();
    const raw = localStorage.getItem(key);
    const log = raw ? JSON.parse(raw).filter(t => now - t < windowMs) : [];

    if (log.length >= maxPerWindow) {
      const resetIn = Math.ceil((log[0] + windowMs - now) / 1000);
  ...
  // ── Ticket purchase guard ──────────────────────────────────
  // Prevents double-clicks and race conditions on Buy button
  const purchaseLocks = new Set();

  async function purchaseWithLock(eventId, purchaseFn) {
    if (purchaseLocks.has(eventId)) {
      throw new Error('Purchase already in progress for this event. Please wait.');
    }

    purchaseLocks.add(eventId);
    try {
      const result = await purchaseFn();
      return result;
    } finally {
      purchaseLocks.delete(eventId);
    }
  }

  // ── Viral moment protection ────────────────────────────────
  // Exponential backoff on repeated failed requests
  const backoffState = new Map();

  async function withBackoff(key, fetchFn, maxRetries) {
    const state = backoffState.get(key) || { failures: 0, nextRetry: 0 };

    if (Date.now() < state.nextRetry) {
      const wait = Math.ceil((state.nextRetry - Date.now()) / 1000);
      throw new Error(`Service busy. Retrying in ${wait}s`);
    }

    try {
      const result = await fetchFn();
      backoffState.delete(key); // reset on success
      return result;
    } catch(err) {
      state.failures = Math.min((state.failures || 0) + 1, maxRetries || 5);
      // Exponential: 1s, 2s, 4s, 8s, 16s
      state.nextRetry = Date.now() + Math.pow(2, state.failures) * 1000;
      backoffState.set(key, state);
      throw err;
    }
  }

  return { checkLimit, purchaseWithLock, withBackoff };
